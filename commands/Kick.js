...
module.exports.run = async (client, message, args) => {
    ...
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply("❌Voor dit commando heb je `KICK_MEMBERS` nodig.");

    if (!message.guild.me.hasPermission("KICK_MEMBERS")) return message.reply("❌Voor dit commando heb je `KICK_MEMBERS` nodig.");

    // if (!args[1]) return message.reply("Geen gebruiker opgegeven.");
    if (!args[0]) return message.reply("❌een gebruiker opgegeven.");
...
module.exports.help = {
    name: "kick",
    category: "Moderator",
    description: "Iemand verwijderen van de server."
}